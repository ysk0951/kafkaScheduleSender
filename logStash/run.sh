...
# ENV
###########################################
JAR="./express-log-sender-0.2.jar"
MODULE_NAME="express-log-sender"
NOW=$(date +"%F")
LOG_DIR=logs
LOG_FILE=server_${NOW}.log
OPT="-Xms1G -Xmx1G -Djgroups.log_class=org.jgroups.logging.Slf4jLogImpl -Djava.net.preferIPv4Stack=true"
DUMP="-Dlogback.configurationFile=./logback.xml -XX:NewRatio=4 -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=50 -XX:+UseCMSInitiatingOccupancyOnly -XX:+DisableExplicitGC -Djava.awt.headless=true"
LOCATION=$(cd ./"$(dirname "$0")" && pwd)
JDK="${LOCATION}/openjdk1.8.0_192"
###########################################
...
stop () {
    ...
    echo " * [$(date +"%F_%H:%M:%S")] : ${MODULE_NAME} RUNNING PROCESS PID : ${PID}"
    echo "stop" > stop.txt
    echo "    => Please Wait Task End...Confirm logs/sys.log"
